...
{
    if(denominator == 0) throw std::invalid_argument("denominator equal zero!!");
    else if(numerator == 0){
    ...
    else
    {
        ...
        int gcd = nod(numerator_, denominator_);
        if(gcd != 0){
            numerator_ /= gcd;
            denominator_ /= gcd;
        }
...
bool Fraction::operator == ( Fraction &other){
    int common = nok(this->denominator_, other.denominator_);
...
Fraction Fraction::operator+(Fraction &other){
    ...
    numt += numo;
    int den = common;
    int NOD = nod(numt,den);
    if(NOD == 0)return Fraction(0, den);
    numt /= NOD;
    den /= NOD;
