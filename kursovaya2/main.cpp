...
void print_to_groundrace(){
    ...
    }while(command != 0 || count_register < 2);
...
void print_to_airgroundrace(){
    ...
    std::vector<std::pair<std::string, double>> resground = Race.get_result(distance);
    std::vector<std::pair<std::string, double>> ressky = Race.get_resultsky(distance);

    std::vector<std::pair<std::string, double>> res = Race.get_resultskyground(resground, ressky);
    for(int i = 0; i < res.size(); i++){
        std::cout << i+1 << ". " << res[i].first << ". Time: " << res[i].second << std::endl;
    }
}
