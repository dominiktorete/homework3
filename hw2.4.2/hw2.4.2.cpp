...
public:
	...
	std::string Addres::get_city() const{
		return city;
	}
	...
	bool compare_city(const Addres& one, const Addres& two){
		return one.get_city() < two.get_city();
	}
	...
	int main(){
		...
		if(file_in.is_open()){
			...
			if(file_out.is_open()){
				...
				std::sort(vec.begin(), vec.end(), compare_city);
